		...
		public void actionPerformed(java.awt.event.ActionEvent e) {
			...
			if(_cnt == _numSteps) {
				if(_reverse) {
					...
					tiles.add(_gameScreen.getTile(_x, _y));
					tiles.add(_gameScreen.getTile(_x, _y - Tile.TILE_SIZE));
					tiles.add(_gameScreen.getTile(_x, _y + Tile.TILE_SIZE));
					tiles.add(_gameScreen.getTile(_x - Tile.TILE_SIZE, _y));
					tiles.add(_gameScreen.getTile(_x + Tile.TILE_SIZE, _y));
					tiles.add(_gameScreen.getTile(_x - Tile.TILE_SIZE, _y - Tile.TILE_SIZE));
					tiles.add(_gameScreen.getTile(_x - 2*Tile.TILE_SIZE, _y - Tile.TILE_SIZE));
					tiles.add(_gameScreen.getTile(_x - 2*Tile.TILE_SIZE, _y));
					tiles.add(_gameScreen.getTile(_x - Tile.TILE_SIZE, _y + Tile.TILE_SIZE));
					tiles.add(_gameScreen.getTile(_x + Tile.TILE_SIZE, _y - Tile.TILE_SIZE));
					
					for (Tile t : tiles) {
						if (t != null && t.isOccupied())
							t.getOccupant().updateHP(-t.getOccupant().getHP()/2);
					}
				}
			}
		}
